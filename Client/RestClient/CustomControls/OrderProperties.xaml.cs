    ...
    public partial class OrderProperties : UserControl
    {
        ...
        private async void StartAnimationAsync()
        {
            await Task.Run(() =>
            {
                ...
                {
                    var heightAnimation = new DoubleAnimation(0, MainGrid.RowDefinitions[1].ActualHeight,
                        new Duration(new TimeSpan(0, 0, 0, 0, 200)));
                    var widthAnimation = new DoubleAnimation(0, MainGrid.ColumnDefinitions[1].ActualWidth,
                        new Duration(new TimeSpan(0, 0, 0, 0, 200)));
